...
Дата проверки: 16 июля 2026. Это рабочий контрольный список: он фиксирует реальное состояние проекта, а не обещает автоматическое одобрение. Решение о выпуске всегда принимают Apple и Google по фактическому бинарнику, метаданным и каталогу.
...
| Защита платного каталога | Есть | iOS отправляет подписанный StoreKit JWS, Android — purchase token Google Play только авторизованному серверу. В production доступ всё ещё скрыт: сервер намеренно не выдаёт entitlement без реальных валидаторов Apple/Google. |
...
| Удаление аккаунта в приложениях | Готово технически | В iOS и Android пользователь явно вводит `DELETE`; сервер удаляет Firebase identity, отзывает сессии и обезличивает связанные данные. Публичная веб-форма с подтверждением по e-mail остаётся дополнительным путём. |
| Нативное HLS-воспроизведение | Подготовлено | AVPlayer и Media3 получают только короткоживущий same-origin gateway URL. Прямые object-storage URL не принимаются клиентами. |
...
- [ ] Подключить выбранный почтовый сервис, `MAILER_WEBHOOK_URL` и домен отправителя для публичной веб-формы поддержки/удаления. Регистрация и подтверждение e-mail в приложениях уже выполняются Firebase Auth; прямое удаление из приложения не зависит от почтового провайдера.
...
- [x] Подключить AVPlayer/Media3 к защищённой HLS-сессии: клиент принимает только короткоживущий same-origin gateway URL, а сервер выдаёт его только для готовой бесплатной HLS-версии.
...
- [x] Реализовать безопасную передачу StoreKit 2 JWS и Google Play purchase token: клиент не открывает доступ и не подтверждает покупку, пока сервер не ответит успешно.
...
Технический UGC-контур уже есть: подтверждённый аккаунт принимает Community Rules перед первой публикацией, доступны жалоба, блокировка автора и очередь модерации Studio. Перед публичным запуском оператор должен утвердить ответственных, канал эскалации и сроки реакции.
