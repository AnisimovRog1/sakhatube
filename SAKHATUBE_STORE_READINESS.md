...
| Удаление аккаунта в приложениях | Подготовлено | В iOS и Android запрос требует совпадения e-mail и подтверждения; фактическое удаление запускает только одноразовая ссылка из письма. |
...
- [ ] Подключить выбранный почтовый сервис, секреты `EMAIL_VERIFICATION_SECRET` / `MAILER_WEBHOOK_URL` и домен отправителя на Railway; нативные экраны регистрации, входа и удаления уже готовы, но без реальной доставки письма production намеренно не выдаст сессию и не примет удаление.
